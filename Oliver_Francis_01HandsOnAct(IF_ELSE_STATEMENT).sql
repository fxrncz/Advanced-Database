DECLARE @ID BIGINT, @LastName VARCHAR(100), @FirstName VARCHAR(100), @Age INT, @Gender VARCHAR(100), @Level VARCHAR(100);
DECLARE RecordsCursor CURSOR FOR SELECT ID, LastName, FirstName, Age, Gender, Level FROM Records;
OPEN RecordsCursor
FETCH NEXT FROM RecordsCursor INTO @ID, @LastName, @FirstName, @Age, @Gender, @Level;
